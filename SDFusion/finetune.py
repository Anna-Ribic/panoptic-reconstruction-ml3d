...
def fine_tune_main_worker(opt, model, train_dl, test_dl, test_dl_for_eval, visualizer, device):
    if get_rank() == 0:
        cprint('[*] Start fine-tuning. name: %s' % opt.name, 'blue')
...
if __name__ == "__main__":
    ...
    fine_tune_main_worker(opt, model, train_dl, test_dl, test_dl_for_eval, visualizer, device)
